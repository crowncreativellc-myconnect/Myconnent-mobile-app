...
import { TrustPathCard } from '../../../src/components/TrustPathCard';
...
export default function ShoutDetailScreen() {
  ...
  const selectedShout = useShoutStore((s) => s.selectedShout);

  const [shout, setShout] = useState<ShoutOut | null>(
    selectedShout?.id === id ? selectedShout : null,
  );
  const [matchedUsers, setMatchedUsers] = useState<UserProfile[]>([]);
  const [isLoading, setIsLoading] = useState(selectedShout?.id !== id);
  ...
  const fetchShout = useCallback(async () => {
    if (!id) return;
    // Already loaded from store — skip network fetch
    if (selectedShout?.id === id) {
      setIsLoading(false);
      return;
    }
    setIsLoading(true);
